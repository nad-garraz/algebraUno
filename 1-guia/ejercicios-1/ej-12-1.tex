\tikzset{
  plot style/.style={
      ...
      eje/.style={thin, Latex-Latex, color=gray},
...
\def\doceiA{
  \begin{tikzpicture}[
    plot style,
    nodo/.style 2 args={color={##1}, label=below:$##2$, outer sep=0, inner sep=0},
    flecha/.style={thick, color=##1, {[-Latex}},
      ]
    \draw[eje] (0,0) -- (12,0);

    \node[nodo={magenta}{5}] (5) at (5,0.2) {};
    \node[nodo={OliveGreen}{8}] (8) at (8,0.3) {};
...
\begin{enumerate}[label=\roman*)]
  ...
  \item
        \begin{enumerate}[label=\alph*)]
          ...
          \item \textit{Si $n$ es un natural que no termina en 4 entonces no es par}.
  ...
  \item
        \begin{itemize}
          \item[e)]
                En la tabla están distintos sabores sobre como leer la proposición. En lenguaje lógico, en notación de conjuntos, gráfico y coso.

                A mí me gusta más en notación de conjuntos, pero lo más claro es hacer un grafiquito, pero ¿Quién me preguntó?
                  {\small
                    $$
                      \begin{array}{|c|c|c|c|}\cline{4-4}
                        \multicolumn{3}{c|}{}   & p \to A \ytext q \to B                                                                                                     \\ \hline
                        p \entonces q           & \paratodo x \en \reales,\, x > 3 \entonces x^2 > 4 & \doceiiicero & A \stacktext{?}{\subseteq} B \Tilde                    \\ \hline
                        \sim q \entonces \sim p & x^2 \leq 4 \entonces x \leq 3                      & \doceiiiuno  & B^c \stacktext{?}{\subseteq} A^c \Tilde                \\ \hline
                        \sim p \lor q           & x \leq 3 \otext x^2 > 4                            & \doceiiidos  & A^c \union B \igual{?} \universo \Tilde                \\ \hline
                        \sim (p ~ ~ \land \sim q)  & \sim (x > 3 \y x^2 \leq 4 )                     & \doceiiitres & (A \inter B^c)^c \igual{?} \vacio^c = \universo \Tilde \\ \hline
                      \end{array}
                    $$
                  }
...
\begin{aportes}[4]
  ...
  \item \aporte{https://github.com/notgaspo}{Gaspar B.K. \github}
\end{aportes}
