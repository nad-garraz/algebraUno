...
\begin{enumerate}[label=\roman*)]
  \item La cantidad de diagonales de un polígono de $n$ lados es $\frac{n(n-3)}{2}$.\par
        ...
        $$
        es verdadera para algún $k \en \enteros_{\geq 3}$.

        Entonces quiero probar que:
        $$
          p(\blue{k+1}): d_{\blue{k+1}} = \frac{(\blue{k+1})(\blue{k + 1} - 3)}{ 2 }
        $$
        también sea verdadera.

        Arranco por la definición de $p(\blue{k+1})$ y busco usar la \purple{hipótesis inductiva} para
        encontrar la expresión buscada:
        $$
          d_{k+1} \igual{def} d_k + k-1
          \igual{\purple{HI}}
        ...
        Como $p(3),\, p(k) \ytext p(k+1)$ resultaron verdaderas, por el principio de inducción $p(n)$ es verdadera $\paratodo n \en \naturales_{\geq 3}$
