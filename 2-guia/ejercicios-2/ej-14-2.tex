...
\begin{enumerate}[label=\roman*)]
  ...
  \item la suma de los ángulos interiores de un polígono de $n$ lados es $\pi(n-2)$.\\
        ...
        Es más claro en este caso que al sacarle un lado, estoy robádo un triángulo
        que tiene como suma de sus ángulos internos $\pi$, entonces afirmo
        $\alpha_{n+1} = \alpha_n + \pi $. Ahora pruebo por inducción lo pedido.
        $p(n): \alpha_n = \pi(n-2) \paratodo n \geq 3$\par
        $ \llave{l}{
            \textit{Caso Base: } p(3): \pi(3-2) = \pi, \text{ lo cual es verdad para el triángulo.\Tilde}\\
            \textit{Paso inductivo: } p(k) \text{ es verdadero para algún } k \en \enteros_{\geq 3}
            \entonces p(k+1) \V?\\
            \textit{Hipótesis Inductiva: } \alpha_k = \pi(k-2) \entonces \alpha_{k+1} = \pi(k-1) \\
            \llaves{l}{
              \alpha_k \igual{def}  \alpha_{k+1} - \pi\\
              \alpha_k \igual{HI}  \pi(k-2)\\
