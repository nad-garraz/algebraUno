...
\begin{enumerate}[label=\roman*)]
  ...
  \item
        ...
        Si te quedaste ahí pedaleando en el aire en el \red{!!!} es solo acomodar
        la expresión, nada raro, algún factor común, $\ub{(-1)^2 = 1}{\text{\tiny \faIcon{meh-rolling-eyes}}}$ y coso... \href{\justDoIt}{power!}.
  ...
  \item
        ...
        $$
          p(n):
          \sumatoria{i=1}{n} \frac{i2^i}{(i + 1)(i+2)} = \frac{2^{n+1}}{n+2} - 1 \paratodo n \en \naturales
        $$

        \textit{Caso base: }
        $$
          p(1) :
          \sumatoria{i=1}{1} \frac{i2^i}{(i + 1)(i+2)} = \frac{1*2^1}{(1+1)(1+2)} = \frac{2}{6} = \frac{1}{3} = \frac{2^2 - 3}{1 + 2}
        $$
        ...
        $$
          p(\blue{h}) :
          \ub{
            \sumatoria{i=1}{\blue{h}} \frac{i2^i}{(i + 1)(i+2)} = \frac{2^{\blue{h}+1}}{\blue{h}+2} -1
          }{
        ...
        $$
          p(\blue{h+1}) :
          \sumatoria{i=1}{\blue{h+1}} \frac{i2^i}{(i + 1)(i+2)} = \frac{2^{(\blue{h+1})+1}}{(\blue{h+1}) + 2} - 1
        $$
        también lo sea.

        Arranco por el paso $h+1$ y busco usar la \purple{hipótesis inductiva}:
        $$
          \begin{array}{rcl}
            \sumatoria{i=1}{\blue{h+1}} \frac{i2^i}{(i + 1)(i+2)}
             & =                    &
            \frac{(h+1)2^{h+1}}{((h+1) + 1)((h+1)+2)}  +  \sumatoria{i=1}{\blue{h}} \frac{i2^i}{(i + 1)(i+2)} \\
             & \igual{\magenta{HI}} &
            \frac{(h+1) 2^{h+1}}{(h + 2)\cdot (h + 3)} + \frac{2^{\blue{h}+1}}{\blue{h}+2} - 1                \\
             & =                    &
            \frac{(h+1) 2^{h+1} + (h+3) \cdot 2^{h+1}}{(h+2)\cdot (h+3)} - 1                                  \\
             & =                    &
            \frac{(2h+4) 2^{h+1}}{(h+2)\cdot (h+3)} - 1                                                       \\
             & \igual{\red{!!}}     &
            \frac{2^{h+2}}{h+3} - 1                                                                           \\
          \end{array}
        $$
        En el \red{!!} saqué factor común y usé propiedades de potencias para el exponente, nada raro.
  ...
  \item
        ...
        también lo sea.\par
        Arranco de $p(k+1)$ y usando la \purple{hipótesis inductiva} trato de llegar
        al valor esperado:
        $$
        ...
        $$
        Si te quedaste pedaleando en los \red{!}, pensá en \textit{diferencia de cuadrados}, \textit{propiedades de potencias} y coso \faIcon{hands-wash}.\par
  ...
  \item
        ...
        para acomodar los factores y así formar la HI, suele ser el camino a seguir, \red{ojo con modificar el término general}.\par
