...
En el enunciado hay 2 cosas:
$$
  \textit{Lo que tengo que encontrar:} (a^{97} - 36 : 135)
  \ytext
  \textit{Restricción de los valores que puede tomar $a$:} (a^{197} - 26 : 15) = 1
$$
Como $135 = 3^3 \cdot 5$, los \ul{posibles valores sin tener en cuenta la restricción} que puede tomar el MCD serían:
$$
  \llamada1 \text{MCD} \en \set{1, 3, 5, 9, 15, 27, 45, 135}
$$.

Por otro lado laburando la restricción tengo que:
$$
...
\textit{Divisibilidad por 3:}
\parrafoDestacado{
  \it    ¿Es la expresión $a^{97} - 36$ divisible por 3 para alguno de los valores de $a$ permitidos?
}
...
\textit{Divisibilidad por 5:}
\parrafoDestacado{
  \it
  ¿Es la expresión $a^{97} - 36$ divisible por 5 para alguno de los valores de $a$ permitidos?
}
$$
...
$$
  \llamada1 \text{MCD} \en \set{1,3,9,27}
$$

Teniendo en cuenta que los posibles MCD, son todos potencias de 3 y que $\blue{\congruencia{a}{0}{3}}$

\bigskip

\textit{Divisibilidad por 9:}
\parrafoDestacado{
  \it
  ¿Es la expresión $a^{97} - 36$ divisible por 9 para alguno de los valores de $a$ permitidos?
}
$$
  \congruencia{a^{97} - 36 }{0}{9}
  \Sii{$\blue{\congruencia{a}{0}{3}}$}[$\Rightarrow a = \blue{3k}$]
  \congruencia{(\blue{3k})^{97} \igual{\red{!!}} 3 \cdot (\ua{\blue{3^2})}{9}^{48} \cdot k^{97}}{0}{9}
  \sii
  \congruencia{0}{0}{9}
$$

Se concluye que la expresión $a^{97} - 36$ es divisible por 9 para los $a$ que cumplen $\congruencia{a}{0}{3}$.
De esa manera \ul{destronando al 3 como posible MCD}.

Teniendo en cuenta que hasta el momento que el 9 es el MCD.

\bigskip

\textit{Divisibilidad por 27:}
\parrafoDestacado{
  \it
  ¿Es la expresión $a^{97} - 36$ divisible por 27 para alguno de los valores de $a$ permitidos?
}
$$
...
Se redujieron los potenciales MCD a:
$$
  \llamada1 \text{MCD} \en \set{1,9}
$$
