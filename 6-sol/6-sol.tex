	...
	\item Calcular $1 + w^2 + w^{-2} + w^4 + w^{-4}$ para cada $w\en G_{10}$.
	      ...
	      \begin{minipage}{0.4\textwidth}
		      ...
		      \red{No tiene al 1? No es $G_5$\\
			      Solo vale el primer gráfico?\\
			      Condiciona los $w$ del ejercicio?}
	      \end{minipage}
...
\begin{minipage}{0.8\textwidth}
	...
	$p(n) :
		...
		\textit{Caso base: }\\
		\llave{l}{
			p(1) : z_1 = 1 + e^{\frac{2\pi}{3}i} = e^{\frac{\pi}{3}i} \Tilde\\
	...
	por criterio de inducción se concluye que $p(n)$ también lo es $\paratodo n \en \naturales$.\\
	Dado que la sucesión $z_n$   tiene solo 2 imágenes, para cualquier $n\en \naturales$ se concluye que
	$z_n \en G_6\ \paratodo n \en \naturales$
\end{minipage}
\begin{minipage}{0.2\textwidth}
	\begin{tikzpicture}[baseline=0,scale = 1.4, every node/.style={font=\tiny}]
		\draw[ultra thin,->,gray] (-1.5,0) -- (1.5,0) node[below] {Re};
		...
		\node[right] at (1*360/6:1)  {$e^{\frac{2\pi}{6}i}$} ;
		\node[right] at (5*360/6:1)  {$e^{-\frac{2\pi}{6}i}$} ;
		\foreach \x in {0,...,5} {
				\filldraw (\x*360/6:1) circle (1pt) ;
				\filldraw (\x*360/6:1.2) node { $\x$};
				\ifnum\x<6
	...
	\end{tikzpicture}
\end{minipage}
\red{corroborar que no sea un delirio}
...
\begin{minipage}{0.6\textwidth}
	Si $z \y w \en G_{20} \sisolosi
		\llave{l}{
			z_k = e^{i \frac{2\pi}{20}k_z }\\
			z_w = e^{i \frac{2\pi}{20}k_w }
		}$ con $r_{20}(k_{\set{z,w}}) \en \enteros_{[0,19]}$\\
	$G_2 = \set{-1, 1} \entonces zw^9 =
		e^{i \frac{2\pi}{20}(k_z + 9 k_w)} \en G_2 \sisolosi\\
		\sisolosi \frac{1}{10}(k_z + 9 k_w)\cancel\pi = k\cancel\pi \sisolosi
		\boxed{k_z = 10k - 9 k_w }$ con $k\en \enteros$\\
	Entonces si $z_{k_z} = e^{i \frac{2\pi}{20}k_z} =
		e^{i \frac{2\pi}{20}(10k - 9 k_w)}$ se tiene que\\
	$z \cdot w^9 =
		e^{i \frac{2\pi}{20}(10k - 9 k_w)} \cdot e^{i \frac{18}{20} k_w} =
		e^{i k\pi} =
		\llave{rl}{
			1 & \text{si $k$ par} \\
			-1 & \text{si $k$ impar} \\
		}$\\
	\red{No le cazo la onda. Entiendo que si $w^k, con k=2 \divideA 20 \entonces w\en G_{10}$
		¿y entonces?
		Está bien pedir esa condición para que se relacionen? }
\end{minipage}
\begin{minipage}{0.3\textwidth}
	\begin{tikzpicture}[baseline=0,scale = 2.5, every node/.style={font=\tiny}]
		\draw[ultra thin,->,gray] (-1.5,0) -- (1.7,0) node[below] {Re};
		\draw[ultra thin,->,gray] (0,-1.5) -- (0,1.7) node[right] {Im};
		\draw[ultra thin] (0,0) circle (1);
		% \filldraw[thin] (-.5,0) circle (1pt) node[below] {$-\frac{1}{2}$};
		% \draw[latex-latex, blue] (-.5,0.1) -- (0.5,0.1) node[midway, above]{ 1};
		% \node[right] at (1*360/20:1)  {$e^{\frac{2\pi}{6}i}$} ;
		% \node[right] at (5*360/20:1)  {$e^{-\frac{2\pi}{6}i}$} ;
		\foreach \x in {0,...,19} {
	...
	\item
	      \begin{itemize}
		      \item
		            \begin{minipage}{0.7\textwidth}
			            \textit{Reflexividad: }
			            Si $ z = e^{i\frac{1}{10}\pi k_z} \entonces
				            z \relacion z \sisolosi
				            k_z \cancel\pi= k\cancel\pi $ \Tilde
