...
\begin{enumerate}[label=\roman*)]
	\item Dado un $z = r\cdot e^{i\theta}$, en forma exponencial $\re{z} >0 \sisolosi \theta = 2k\pi$ con $k \en \enteros$
	      \begin{itemize}
	...
	\item
	      arg$( 1 + i) = \frac{\pi}{4}$ los $w \en \complejos$ tales que $z \relacion w \sisolosi \text{arg}(w) =\frac{1}{4}\pi$ \\
	      \begin{tikzpicture}[baseline=0,scale = 1.4, every node/.style={font=\tiny}]
		      \draw[ultra thin,->,gray] (-1.5,0) -- (1.5,0) node[below] {Re};
		      \draw[ultra thin,->,gray] (0,-1.5) -- (0,1.5) node[right] {Im};
		      \draw[magenta] (0,0) -- (2,2) node[right] {$\clase{z}$};
		      % \draw[ultra thin] (0,0) circle (1);
		      \filldraw[thin] (1,1) circle (1pt) node[below, right] {$z$}; % Added the origin
