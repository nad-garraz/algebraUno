...
\begin{itemize}
  ...
  \item \textit{Máximo común divisor:}\par
        ...
        \begin{itemize}[label = \tiny \faIcon{meh}]
          \item $(f:g) \divideA f \quad \ytext \quad (f:g) \divideA g$

          \item $f = (f:g) \cdot k_f \quad \ytext \quad g = (f:g) \cdot k_g \quad \text{con} \quad k_f \quad \ytext \quad  k_g$ en $\K[X]$

          \item Dos polinomios son coprimos si $(f:g) = 1 \sisolosi f \distinto g$
        \end{itemize}

  \item \textit{Raíces múltiples:}\par
        ...
        \begin{itemize}[label=\tiny \faIcon{meh}]
          \item Cuando $f$ tiene una raíz múltiple:
                $$
                  \begin{array}{c}
                    ...
                    f = (X - \alpha)\cdot q,\ \text{ con } q \en \K[X] \quad \ytext \quad \magenta{ q(\alpha) = 0}.
                  \end{array}
                $$
          ...
          \item Cuando la raíz \textit{no} es múltiple, es \textit{simple} cuando:
                $$
                  \begin{array}{c}
                    ...
                    f = (X - \alpha)\cdot q,\ \text{ con } q \en \K[X] \quad \ytext \quad \magenta{ q(\alpha) \distinto 0}.
                  \end{array}
                $$
          ...
          \item Sea $m \en \naturales_0$. Se dice que $\alpha$ es raíz de multiplicidad (exactamente)
                ...
                O equivalentemente,
                $$
                  f = (X - \alpha)^m q \quad \text{con} \quad q \en \K[X], \quad \ytext \quad \magenta{q (\alpha) \distinto 0}.
                $$
          ...
          \item $\alpha$ es raíz múltiple de $f$ si y solo si:
                $$
                  f(\alpha) = 0 \quad \ytext \quad f'(\alpha) =
